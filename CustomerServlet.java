...
public class CustomerDAO {
    ...
    public int getTotalRecords(String name, String sex, String birthdayFrom, String birthdayTo) {
        ...
        // Kiểm tra sex có giá trị hợp lệ
        if (sex != null && !sex.trim().isEmpty() && !"-1".equals(sex.trim())) {
            sql.append(" AND Sex = ?");
            params.add(sex.trim());
        }

        // Kiểm tra birthdayFrom
    ...
    public List<CustomerDTO> searchCustomers(String name, String sex, String birthdayFrom, String birthdayTo, int page) {
        ...
        StringBuilder sql = new StringBuilder(
            ...
            + " FROM MSTCUSTOMER WHERE DeleteYMD IS NULL");
        ...
        // Xử lý phân trang
        int offset = (page - 1) * RECORDS_PER_PAGE + 1;
        int limit = page * RECORDS_PER_PAGE;
        sql.append(") AS ResultSet WHERE RowNum BETWEEN ? AND ?");
        params.add(offset);
        params.add(limit);
    ...
    public boolean deleteCustomers(String[] customerIds) {
        String sql = "UPDATE MSTCUSTOMER SET DeleteYMD = GETDATE() WHERE Id IN (";
    ...
    <c:if test="${maxPage > 1}">
        <div style="text-align: center; margin-top: 20px;">
            <form action="search.jsp" method="get">
                ...
                <button type="submit" name="page" value="${maxPage}" 
                        ${currentPage == maxPage ? 'disabled' : ''}>
                    Last
                </button>
            </form>
...
public class CustomerDAO {
    ...
    public List<CustomerDTO> searchCustomers(String name, String sex, String birthdayFrom, String birthdayTo, int page) {
        ...
        // Thêm ORDER BY và phân trang sử dụng OFFSET-FETCH
